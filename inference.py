    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            for newPos, prob in NewDistr.items():
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for pos in self.legalPositions:
                ...
                allPossible[pos] = all_p[pos] + (emissionModel[dist] * old_belief[pos])
            if not any(all_p.values()):
                self.initializeUniformly(gameState)
            else:
                ls = []
                for x in range(0, self.numParticles):
                    ls.append(util.sample(allPossible))
                self.particles = ls
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [util.sample(\
            self.getPositionDistribution(self.setGhostPosition(gameState, part))) \
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        curr_belief = util.Counter()
        for part in self.particles:
            curr_belief[part] += 1
        curr_belief.normalize()
        return curr_belief
...
class JointParticleFilter:
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                n_dist = getPositionDistributionForGhost(\
                    setGhostPositions(gameState, newParticle)\
                    , i, self.ghostAgents[i])
                newParticle[i] = util.sample(n_dist)
            "*** END YOUR CODE HERE ***"
